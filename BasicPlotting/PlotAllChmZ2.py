...
def createDataH1(ch,channels):
    ...
    if (ZZ4lSelection):
        if (ch=="eeee"):
            ...
            chain.Draw("(( abs(e1_e2_Mass-91.1876)<abs(m1_m2_Mass-91.1876) ) ? m1_m2_Mass : e1_e2_Mass)>>"+data_hist,"(nZZTightIsoElec+nZZTightIsoMu==4) && (Mass<115 || Mass>150) &&"+ZZ4lselection)
    else:
        ...
        #Function to retrieve mZ1 in ch=eemm
            ...
            chain.Draw("(( abs(e1_e2_Mass-91.1876)<abs(m1_m2_Mass-91.1876) ) ? m1_m2_Mass : e1_e2_Mass)>>"+data_hist, "(nZZTightIsoElec+nZZTightIsoMu==4) && (Mass<115 || Mass>150)") 
...
def createMCStack(ch,channels,leg):
    ...
    for plotgroup in plotgroups.keys():
        ...
        mc_Group = "Z2Mass_"+ch+"_"+plotgroup
        ...
        for mc_Sample in members:
            ...
            if (ZZ4lSelection):
                if (ch=="eeee"):
                    ...
                    ZZ4lselection=selvar[0]+">60 &&"+selvar[0]+"<120 &&"+selvar[2]+">60 &&"+selvar[2]+"<120)"
                    MCchain.Draw("(( abs(e1_e2_Mass-91.1876)<abs(m1_m2_Mass-91.1876) ) ? m1_m2_Mass : e1_e2_Mass)>>"+hist_name,"genWeight*((nZZTightIsoElec+nZZTightIsoMu==4) && (Mass<115 || Mass>150) &&"+ZZ4lselection)
            else:
                if (ch=="eeee" or ch=="mmmm"):
                    MCchain.Draw(channels[ch]+">>"+hist_name,"genWeight*((nZZTightIsoElec+nZZTightIsoMu==4) && (Mass<115 || Mass>150))")
                #Function to retrieve mZ1 in ch=eemm
                else:
                    MCchain.Draw("(( abs(e1_e2_Mass-91.1876)<abs(m1_m2_Mass-91.1876) ) ? m1_m2_Mass : e1_e2_Mass)>>"+hist_name, "genWeight*((nZZTightIsoElec+nZZTightIsoMu==4) && (Mass<115 || Mass>150))") 
...
def stackplot(ch,channels):
    ...
    c.Update()
    if (ZZ4lSelection): 
        c.SaveAs("DataMCPlots_Mar29/Z2Mass_ZZ4lSelection_"+ch+"_blinded_m4l_"+Date+".pdf")  
    else:
        c.SaveAs("DataMCPlots_Mar29/Z2Mass_FullSpectrum_"+ch+"_blinded_m4l_"+Date+".pdf") 
    #text = raw_input()
